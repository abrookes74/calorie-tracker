...
let session=null,selectedDate=new Date(),profile={daily_calorie_target:2000},foods=[],recipes=[],recipeItems=[],diary=[],weights=[],recentDiary=[];
...
mealSections:$("#mealSections"),quickAddList:$("#quickAddList"),recipeDetailDialog:$("#recipeDetailDialog"),recipeDetailName:$("#recipeDetailName"),recipeDetailMeta:$("#recipeDetailMeta"),recipeDetailIngredients:$("#recipeDetailIngredients"),recipeDetailInstructions:$("#recipeDetailInstructions"),foodList:$("#foodList"),foodSearch:$("#foodSearch"),foodCategoryFilter:$("#foodCategoryFilter"),recipeList:$("#recipeList"),recipeSearch:$("#recipeSearch"),recipeLabelFilter:$("#recipeLabelFilter"),
...
async function handleSession(){const yes=!!session?.user;E.authView.hidden=yes;E.appView.hidden=!yes;if(!yes)return;E.accountEmail.textContent=session.user.email||"";await ensureProfile();await refreshCore();await refreshDiary();await refreshRecentDiary();await refreshWeights()}
...
async function refreshRecentDiary(){
  const r=await client.from("diary_entries").select("*").eq("user_id",session.user.id).order("created_at",{ascending:false}).limit(60);
  if(r.error)throw r.error;recentDiary=r.data||[];renderQuickAdd();
}
...
function renderQuickAdd(){
  if(!E.quickAddList)return;
  const seen=new Set(),items=[];
  for(const d of recentDiary){const key=`${d.item_type}:${d.item_ref_id}`;if(!d.item_ref_id||seen.has(key))continue;seen.add(key);items.push({kind:d.item_type,id:d.item_ref_id,name:d.item_name,cal:Number(d.calories)/(Number(d.quantity)||1)});if(items.length>=8)break;}
  for(const f of foods.filter(x=>x.favourite&&x.calories!=null)){const key=`food:${f.id}`;if(seen.has(key))continue;seen.add(key);items.push({kind:"food",id:f.id,name:f.name,cal:Number(f.calories)});if(items.length>=10)break;}
  E.quickAddList.innerHTML=items.length?items.map(x=>`<button class="quick-card" data-quick-kind="${x.kind}" data-quick-id="${x.id}"><strong>${esc(x.name)}</strong><span>${fmt(x.cal)} kcal</span></button>`).join(""):`<div class="empty">Your recent and favourite items will appear here.</div>`;
}
function renderDiary(){
  renderQuickAdd();
  const meals=["Breakfast","Lunch","Dinner","Snacks"];
  E.mealSections.innerHTML=meals.map(m=>{const rows=diary.filter(x=>x.meal_type===m),total=rows.reduce((s,x)=>s+Number(x.calories),0);return `<section class="meal-section"><div class="meal-heading"><h3>${m}</h3><div class="meal-heading-actions"><span>${fmt(total)} kcal</span><button class="copy-meal-btn" data-action="copy-prev-meal" data-meal="${m}">Copy</button><button class="meal-add-btn" data-action="add-meal" data-meal="${m}">+ Add</button></div></div><div class="meal-card">${rows.length?rows.map(x=>`<div class="diary-row"><div><div class="name">${esc(x.item_name)}</div><div class="meta">${esc(x.quantity_label||"")}</div></div><div class="calories">${fmt(x.calories)} kcal</div><div><button class="small-btn" data-action="edit-diary" data-id="${x.id}">Edit</button><button class="remove-btn" data-action="remove-diary" data-id="${x.id}">✕</button></div></div>`).join(""):`<div class="empty">Nothing added.</div>`}</div></section>`}).join("");
}
...
function renderFoods(){
  ...
  const list=foods
    .filter(f=>(!q||f.name.toLowerCase().includes(q))&&(!cat||f.category===cat))
    .sort((a,b)=>(Number(b.favourite)-Number(a.favourite))||new Date(b.last_used_at||0)-new Date(a.last_used_at||0)||a.name.localeCompare(b.name));

  E.foodList.innerHTML=list.length?list.map(f=>`
    <div class="item-card">
      <div>
        <div class="name">${f.favourite?'<span class="star">★</span> ':""}${esc(f.name)}
          <span class="badge">${esc(f.category||"Other")}</span>
          <span class="shared-badge">Shared</span>
        </div>
        <div class="meta">${f.calories!==null&&f.calories!==undefined?`${fmt(f.calories)} kcal per ${f.base_amount} ${esc(f.unit)}`:"Calories not set"}${f.serving_description?" • "+esc(f.serving_description):""}</div>
      ...
      <div class="calories">${f.calories!==null&&f.calories!==undefined?fmt(f.calories)+" kcal":""}</div>
...
function renderRecipeLabels(){
  ...
  E.recipeLabelFilter.innerHTML=`<option value="">All recipe labels</option>`+labels.map(l=>`<option value="${esc(l)}" ${l===current?"selected":""}>${esc(l)}</option>`).join("");
  const diaryCurrent=E.diaryRecipeLabelFilter.value;
  E.diaryRecipeLabelFilter.innerHTML=`<option value="">All recipe labels</option>`+labels.map(l=>`<option value="${esc(l)}" ${l===diaryCurrent?"selected":""}>${esc(l)}</option>`).join("");
  const chips=$("#recipeLabelChips");if(chips)chips.innerHTML=`<button class="label-chip ${current?"":"active"}" data-recipe-label="">All</button>`+labels.map(l=>`<button class="label-chip ${l===current?"active":""}" data-recipe-label="${esc(l)}">${esc(l)}</button>`).join("");
}
function openRecipeDetail(r){
 const items=recipeItems.filter(x=>x.recipe_id===r.id);E.recipeDetailName.textContent=r.name;E.recipeDetailMeta.innerHTML=`${r.servings} servings • ${recipePerServing(r)?fmt(recipePerServing(r))+" kcal/serving":"Calories not set"}<br>${(r.categories||[]).map(x=>`<span class="badge">${esc(x)}</span>`).join(" ")}`;E.recipeDetailIngredients.innerHTML=`<strong>Ingredients</strong><br>${items.length?items.map(x=>esc(x.raw_text||"")).join("<br>"):"No ingredients stored."}`;E.recipeDetailInstructions.innerHTML=`<strong>Instructions</strong><br>${esc(r.instructions||"No instructions stored.")}`;E.recipeDetailDialog.showModal();
}
function renderRecipes(){
  const q=(E.recipeSearch.value||"").trim().toLowerCase();
  const label=E.recipeLabelFilter.value||"";
  const list=recipes
    .filter(r=>!q||r.name.toLowerCase().includes(q))
    .filter(r=>!label||(Array.isArray(r.categories)&&r.categories.includes(label)))
    .slice()
  ...
  E.recipeList.innerHTML=list.length?list.map(r=>{
    ...
    return `<div class="item-card">
      <div>
        <div class="name"><button class="recipe-name-link" data-action="view-recipe" data-id="${r.id}">${esc(r.name)}</button> <span class="shared-badge">Shared</span></div>
        <div class="meta">${r.servings} servings ${labels}</div>
        <div class="item-actions">
          <button class="small-btn" data-action="edit-recipe" data-id="${r.id}">${per?"Edit":"Set calories"}</button>
          <button class="small-btn" data-action="delete-recipe" data-id="${r.id}">Delete</button>
        </div>
      </div>
      <div class="calories">${per?fmt(per)+" kcal/serving":"Calories not set"}</div>
    </div>`;
...
E.recipeSearch.oninput=renderRecipes;E.recipeLabelFilter.onchange=()=>{renderRecipeLabels();renderRecipes()};$("#recipeLabelChips").onclick=e=>{const b=e.target.closest("[data-recipe-label]");if(!b)return;E.recipeLabelFilter.value=b.dataset.recipeLabel;renderRecipeLabels();renderRecipes()}
...
function addQuantityShortcuts(){let wrap=$("#quantityShortcuts");if(!wrap){wrap=document.createElement("div");wrap.id="quantityShortcuts";wrap.className="quantity-shortcuts";E.diaryQuantity.parentElement.insertAdjacentElement("afterend",wrap)}const o=E.diaryItemSelect.selectedOptions[0];if(!o||!o.value){wrap.innerHTML="";return}const unit=o.dataset.unit;const vals=(unit==="g"||unit==="ml")?[50,100,150,200]:[0.5,1,1.5,2];wrap.innerHTML=vals.map(v=>`<button type="button" class="qty-btn" data-qty="${v}">${v}${unit==="g"||unit==="ml"?unit:"×"}</button>`).join("");wrap.querySelectorAll(".qty-btn").forEach(b=>b.onclick=()=>{E.diaryQuantity.value=b.dataset.qty;updateDiaryCalc()})}
$("#addDiaryBtn").onclick=()=>{E.diaryEditId.value="";$("#diaryDialogTitle").textContent="Add to diary";E.diaryItemTypeFilter.value="food";E.diaryCategoryFilter.value="";E.diaryRecipeLabelFilter.value="";diaryOptions();E.diaryQuantity.value=1;updateDiaryCalc();E.addDiaryDialog.showModal();addQuantityShortcuts()}
...
E.diaryItemSelect.onchange=()=>{const o=E.diaryItemSelect.selectedOptions[0];E.diaryQuantity.value=["g","ml"].includes(o?.dataset.unit)?o.dataset.base:1;updateDiaryCalc();addQuantityShortcuts()};E.diaryQuantity.oninput=updateDiaryCalc
$("#addDiaryForm").onsubmit=async e=>{if(e.submitter?.value!=="add")return;e.preventDefault();const [kind,id]=E.diaryItemSelect.value.split(":"),q=Number(E.diaryQuantity.value);let name,cal,label;if(kind==="food"){const f=foods.find(x=>x.id===id);name=f.name;cal=Number(f.calories)*(q/Number(f.base_amount));label=`${q} ${f.unit}`;await client.from("foods").update({last_used_at:new Date().toISOString()}).eq("id",f.id)}else{const r=recipes.find(x=>x.id===id);name=r.name;cal=recipePerServing(r)*q;label=`${q} serving${q===1?"":"s"}`}const p={user_id:session.user.id,entry_date:dateKey(selectedDate),meal_type:E.diaryMeal.value,item_type:kind,item_ref_id:id,item_name:name,quantity:q,quantity_label:label,calories:cal};const r=E.diaryEditId.value?await client.from("diary_entries").update(p).eq("id",E.diaryEditId.value):await client.from("diary_entries").insert(p);if(r.error)return alert(r.error.message);E.addDiaryDialog.close();await refreshCore();await refreshDiary();await refreshRecentDiary()}
async function copyPreviousMeal(meal){const d=new Date(selectedDate);d.setDate(d.getDate()-1);const r=await client.from("diary_entries").select("*").eq("user_id",session.user.id).eq("entry_date",dateKey(d)).eq("meal_type",meal);if(r.error)return alert(r.error.message);if(!r.data.length)return alert(`No ${meal.toLowerCase()} entries on the previous day.`);const rows=r.data.map(x=>({user_id:session.user.id,entry_date:dateKey(selectedDate),meal_type:x.meal_type,item_type:x.item_type,item_ref_id:x.item_ref_id,item_name:x.item_name,quantity:x.quantity,quantity_label:x.quantity_label,calories:x.calories}));const ins=await client.from("diary_entries").insert(rows);if(ins.error)return alert(ins.error.message);await refreshDiary();await refreshRecentDiary()}
E.mealSections.onclick=async e=>{const b=e.target.closest("button");if(!b)return;if(b.dataset.action==="add-meal"){E.diaryEditId.value="";$("#diaryDialogTitle").textContent=`Add to ${b.dataset.meal}`;E.diaryMeal.value=b.dataset.meal;E.diaryItemTypeFilter.value="food";E.diaryCategoryFilter.value="";E.diaryRecipeLabelFilter.value="";diaryOptions();E.diaryQuantity.value=1;updateDiaryCalc();E.addDiaryDialog.showModal();addQuantityShortcuts();return}if(b.dataset.action==="copy-prev-meal"){await copyPreviousMeal(b.dataset.meal);return}const x=diary.find(d=>d.id===b.dataset.id);if(b.dataset.action==="remove-diary"){const r=await client.from("diary_entries").delete().eq("id",b.dataset.id);if(r.error)return alert(r.error.message);await refreshDiary();await refreshRecentDiary()}if(b.dataset.action==="edit-diary"&&x){E.diaryEditId.value=x.id;$("#diaryDialogTitle").textContent="Edit diary entry";diaryOptions();E.diaryMeal.value=x.meal_type;E.diaryItemSelect.value=`${x.item_type}:${x.item_ref_id}`;E.diaryQuantity.value=x.quantity;updateDiaryCalc();E.addDiaryDialog.showModal();addQuantityShortcuts()}}
...
E.quickAddList.onclick=e=>{const b=e.target.closest("[data-quick-kind]");if(!b)return;E.diaryEditId.value="";$("#diaryDialogTitle").textContent="Quick add";E.diaryItemTypeFilter.value=b.dataset.quickKind;E.diaryCategoryFilter.value="";E.diaryRecipeLabelFilter.value="";diaryOptions();E.diaryItemSelect.value=`${b.dataset.quickKind}:${b.dataset.quickId}`;E.diaryQuantity.value=1;updateDiaryCalc();E.addDiaryDialog.showModal();addQuantityShortcuts()};
document.querySelectorAll("[data-mobile-tab]").forEach(b=>b.onclick=()=>{document.querySelectorAll("[data-mobile-tab]").forEach(x=>x.classList.remove("active"));b.classList.add("active");document.querySelectorAll(".tab").forEach(x=>x.classList.toggle("active",x.dataset.tab===b.dataset.mobileTab));document.querySelectorAll(".tab-panel").forEach(x=>x.classList.remove("active"));$("#"+b.dataset.mobileTab).classList.add("active")});
