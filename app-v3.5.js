...
diaryMeal:$("#diaryMeal"),diaryItemTypeFilter:$("#diaryItemTypeFilter"),diaryCategoryFilter:$("#diaryCategoryFilter"),diaryCategoryFilterLabel:$("#diaryCategoryFilterLabel"),diaryItemSelect:$("#diaryItemSelect"),diaryQuantity:$("#diaryQuantity"),diaryUnitHint:$("#diaryUnitHint"),diaryCalculatedCalories:$("#diaryCalculatedCalories"),
...
function renderFoods(){const q=E.foodSearch.value.toLowerCase(),cat=E.foodCategoryFilter.value;const list=foods.filter(f=>(!q||f.name.toLowerCase().includes(q))&&(!cat||f.category===cat)).sort((a,b)=>(Number(b.favourite)-Number(a.favourite))||Number(a.calories==null)-Number(b.calories==null)||new Date(b.last_used_at||0)-new Date(a.last_used_at||0)||a.name.localeCompare(b.name));E.foodList.innerHTML=list.length?list.map(f=>{const known=f.calories!==null&&f.calories!==undefined;return `<div class="item-card"><div><div class="name">${f.favourite?'<span class="star">★</span> ':""}${esc(f.name)} <span class="badge">${esc(f.category||"Other")}</span>${f.imported_ingredient?'<span class="badge">RecipeSage ingredient</span>':""}</div><div class="meta">${known?`${fmt(f.calories)} kcal per ${f.base_amount} ${esc(f.unit)}`:"Calories not set"}${f.serving_description?" • "+esc(f.serving_description):""}</div><div class="item-actions"><button class="small-btn" data-action="edit-food" data-id="${f.id}">${known?"Edit":"Set calories"}</button><button class="small-btn" data-action="delete-food" data-id="${f.id}">Delete</button></div></div><div class="calories">${known?fmt(f.calories)+" kcal":"Needs calories"}</div></div>`}).join(""):`<div class="empty">No foods found.</div>`}
...
function renderRecipes(){E.recipeList.innerHTML=recipes.length?recipes.slice().sort((a,b)=>a.name.localeCompare(b.name)).map(r=>{const per=recipePerServing(r),unpriced=recipeItems.filter(x=>x.recipe_id===r.id).filter(it=>{const f=foods.find(x=>x.id===it.food_id);return !f||f.calories===null||f.calories===undefined}).length;return `<div class="item-card"><div><div class="name">${esc(r.name)}</div><div class="meta">${r.servings} servings${(r.categories||[]).length?" • "+esc(r.categories.join(", ")):""}${unpriced?` • ${unpriced} ingredient(s) need calories`:""}</div><div class="item-actions"><button class="small-btn" data-action="edit-recipe" data-id="${r.id}">${per?"Edit":"Set calories"}</button><button class="small-btn" data-action="delete-recipe" data-id="${r.id}">Delete</button></div></div><div class="calories">${per?fmt(per)+" kcal/serving":"Set calories to log"}</div></div>`}).join(""):`<div class="empty">No recipes yet.</div>`}
...
function diaryOptions(){
  ...
  const category=E.diaryCategoryFilter.value||"";
  ...
  const foodOpts=recent
    ...
    .filter(f=>!category||(f.category||"Other")===category)
...
function updateDiaryCalc(){const o=E.diaryItemSelect.selectedOptions[0];if(!o||!o.value){E.diaryCalculatedCalories.textContent="0";E.diaryUnitHint.textContent="";return;const q=Number(E.diaryQuantity.value)||0;E.diaryCalculatedCalories.textContent=fmt(Number(o.dataset.cal)*(q/Number(o.dataset.base)));E.diaryUnitHint.textContent=`Enter quantity in ${o.dataset.unit}.`}
$("#addDiaryBtn").onclick=()=>{E.diaryEditId.value="";$("#diaryDialogTitle").textContent="Add to diary";E.diaryItemTypeFilter.value="food";E.diaryCategoryFilter.value="";diaryOptions();E.diaryQuantity.value=1;updateDiaryCalc();E.addDiaryDialog.showModal()}
...
$("#buildIngredientFoodsBtn").onclick=async()=>{
  const candidates=recipeItems.filter(it=>!it.food_id && it.raw_text && !/:\s*$/.test(it.raw_text));
  if(!candidates.length){alert("There are no unmatched imported ingredients to convert into foods.");return;}
  if(!confirm(`Create/link foods from ${candidates.length} unmatched recipe ingredient lines? Foods will start with calories unset.`))return;
  const cache=new Map(foods.map(f=>[(f.normalized_name||String(f.name).toLowerCase()),f]));
  let linked=0,startFoods=foods.length,failed=0;
  try{
    for(let i=0;i<candidates.length;i++){
      const it=candidates[i];
      const f=await getOrCreateIngredientFood(it.raw_text,cache);
      if(!f)continue;
      const parsed=ingredientUnitAndAmount(it.raw_text);
      const up=await client.from("recipe_items").update({food_id:f.id,amount:parsed.amount||1}).eq("id",it.id);
      if(up.error){failed++;continue;}
      linked++;
    }
    await refreshCore();
    alert(`Done. ${Math.max(0,foods.length-startFoods)} new foods created and ${linked} recipe ingredient lines linked.${failed?` ${failed} lines could not be linked.`:""}`);
  }catch(err){
    console.error(err);alert("Backfill stopped: "+err.message);
  }
}
...
function normaliseIngredientName(raw){
  ...
  s=s.replace(/[¼½¾⅓⅔⅛⅜⅝⅞]/g," ");
  s=s.replace(/^\s*(?:\d+(?:\.\d+)?|\d+\s*[-–]\s*\d+|\d+\s*\/\s*\d+|x\s*\d+)\s*/i,"");
  s=s.replace(/^\s*(?:x\s*)?\d+\s*/i,"");
  s=s.replace(/^\s*(?:g|kg|ml|l|litres?|liters?|tsp|teaspoons?|tbsp|tablespoons?|cups?|oz|lb|lbs|cloves?|slices?|cans?|tins?|packs?|handfuls?|pinch(?:es)?)\b\s*/i,"");
  s=s.replace(/\([^)]*\)/g," ");
  s=s.split(",")[0];
  s=s.replace(/\b(?:fresh|dried|ground|finely|roughly|chopped|sliced|diced|grated|peeled|crushed|optional|to taste|for serving|to serve)\b/gi," ");
  s=s.replace(/\s+/g," ").trim();
  return s;
}
function ingredientUnitAndAmount(raw){
  const t=String(raw||"").toLowerCase().replace(/,/g,"");
  let m;
  if((m=t.match(/(\d+(?:\.\d+)?)\s*kg\b/))) return {amount:Number(m[1])*1000,unit:"g",base:100};
  if((m=t.match(/(\d+(?:\.\d+)?)\s*g\b/))) return {amount:Number(m[1]),unit:"g",base:100};
  if((m=t.match(/(\d+(?:\.\d+)?)\s*l(?:itre|iter)?s?\b/))) return {amount:Number(m[1])*1000,unit:"ml",base:100};
  if((m=t.match(/(\d+(?:\.\d+)?)\s*ml\b/))) return {amount:Number(m[1]),unit:"ml",base:100};
  const leading=t.match(/^\s*(\d+(?:\.\d+)?)/);
  return {amount:leading?Number(leading[1]):1,unit:"item",base:1};
}
async function getOrCreateIngredientFood(raw,cache){
  const name=normaliseIngredientName(raw);
  if(!name)return null;
  if(cache.has(name))return cache.get(name);
  let existing=foods.find(f=>(f.normalized_name||f.name.toLowerCase())===name);
  if(existing){cache.set(name,existing);return existing;}
  const parsed=ingredientUnitAndAmount(raw);
  const ins=await client.from("foods").insert({
    user_id:session.user.id,
    name:name.replace(/\b\w/g,c=>c.toUpperCase()),
    normalized_name:name,
    calories:null,
    base_amount:parsed.base,
    unit:parsed.unit,
    category:"Imported Ingredients",
    imported_ingredient:true,
    serving_description:"Imported from RecipeSage ingredient"
  }).select().single();
  if(ins.error)throw new Error(`Creating ingredient food "${name}": ${ins.error.message}`);
  foods.push(ins.data);cache.set(name,ins.data);return ins.data;
}
...
E.recipeImportFile.onchange=async()=>{
 ...
 try{
   ...
   let imported=0,skipped=0,ingredientCount=0,foodCreatedStart=foods.length,calorieCount=0;
   const total=recs.filter(r=>r?.["@type"]==="Recipe").length;
   const foodCache=new Map(foods.map(f=>[(f.normalized_name||String(f.name).toLowerCase()),f]));

   E.importSummary.innerHTML=`<p><strong>Importing ${total} recipes and ingredient foods…</strong></p><p id="importProgress">0 of ${total} processed.</p>`;

   for(const r of recs){
     if(r["@type"]!=="Recipe")continue;
     const ext=r.identifier||null;
     if(ext){
       const check=await client.from("recipes").select("id").eq("user_id",session.user.id).eq("external_identifier",ext).maybeSingle();
       if(check.error)throw new Error(`Checking "${r.name||"recipe"}": ${check.error.message}`);
       if(check.data){skipped++;const p=$("#importProgress");if(p)p.textContent=`${imported+skipped} of ${total} processed.`;continue;}
     }

     const importedCals=extractCalories(r);if(importedCals!==null)calorieCount++;
     const payload={
       user_id:session.user.id,name:r.name||"Imported recipe",servings:parseYield(r.recipeYield),
       instructions:instructionText(r.recipeInstructions)||null,source_url:r.isBasedOn||null,
       imported_from:"RecipeSage JSON",external_identifier:ext,
       categories:(Array.isArray(r.recipeCategory)?r.recipeCategory:[]).filter(c=>!/^import on /i.test(c)),
       description:r.description||null,author_notes:authorNotes(r)||null,
     ...
     const rows=[];
     for(const raw of (Array.isArray(r.recipeIngredient)?r.recipeIngredient:[])){
       const text=String(raw).trim();
       const heading=/:\s*$/.test(text);
       let f=null,parsed={amount:1};
       if(!heading){
         f=await getOrCreateIngredientFood(text,foodCache);
         parsed=ingredientUnitAndAmount(text);
       }
       rows.push({user_id:session.user.id,recipe_id:ins.data.id,food_id:f?.id||null,amount:parsed.amount||1,raw_text:text});
       ingredientCount++;
     }
     if(rows.length){
     ...
     imported++;
     const p=$("#importProgress");if(p)p.textContent=`${imported+skipped} of ${total} processed — ${imported} imported, ${skipped} skipped.`;
   }

   await refreshCore();
   const createdFoods=Math.max(0,foods.length-foodCreatedStart);
   E.importSummary.innerHTML=`<p><strong>Import complete.</strong></p>
   <p><strong>${imported}</strong> recipes imported; <strong>${skipped}</strong> already-imported recipes skipped.</p>
   <p><strong>${createdFoods}</strong> reusable ingredient foods created.</p>
   <p><strong>${ingredientCount}</strong> ingredient lines linked/preserved.</p>
   <p><strong>${calorieCount}</strong> recipes had a calorie-per-serving value detected.</p>
   <p>Imported ingredient foods start with <strong>Calories not set</strong>. Set their calories in the Foods tab. A recipe can also be logged immediately once you enter its <strong>Calories per serving</strong> manually.</p>`;
 }catch(err){
   console.error(err);
